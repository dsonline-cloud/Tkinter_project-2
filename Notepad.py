...
def openFile():
    global file
    file = askopenfilename(defaultextension = "txt", filetypes = [("All File", "*.*"), ("Text Documents")]) #File can be of any type and defualt extension of the file is ".txt". This function will help us in acccessing the files from the system.
    #So what will happen above is, we will be select a file and that file will be assigned to the variable "file".

    if file == "": #In case there is not file to open, that means we clicked on open but didn't selected any file to open, in that case file variable will again become "None" that means nothing is there.
        file = None
    else:
        root.title(os.path.basename(file) + "- Notepad") #It will help us in getting the name of the file, it will replace the title of notepad "untitled - Notepad" with "Name Of File -Notepad" while opening the file.
...
def saveFile():
    ...
    if file == None: #If it's a new file hence it isn't saved earlier hence "file = None", So now we will save it as a new file.
        ...
        filetypes = [("All Files", "*.*"), ("Text Documnets", "*.txt")]) #In case we don't provide any file name so "Untitled -Notepad" will be the default name of the file to save the file.
...
if __name__ == '__main__':
    ...
    #Adding text area in notepad where we will be writing the text.
    ...
    #To open a new file.
    FileMenu.add_command(label = "New", command = newFile) #It's a dropdown of File menu which will appear in menu bar.
    ...
    Scroll.config(command = TextArea.yview) #Configuring vertical scroll bar.
